...
module.exports = function(env) {
	...
	return function(req, res, next) {
		...
		// check for force delete required
		if (env._reqForceDel === true && _.lowerCase(_.get(req, 'query.force')) !== 'true') {
			utils.send(res, 400, {
				errorCode: 400,
				message: 'You must supply the force=true query string parameter in order to remove objects'
			});
			return next();
		}
